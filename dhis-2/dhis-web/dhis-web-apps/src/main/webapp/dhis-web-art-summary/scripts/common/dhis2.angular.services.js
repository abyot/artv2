...
.service('DateUtils', function ($filter, CalendarService) {
    ...
    return {
        ...
        formatFromUserToApi: function (dateValue) {
            console.log('received:  ', angular.copy(dateValue));
            if (!dateValue) {
                return;
            }
            var calendarSetting = CalendarService.getSetting();
            dateValue = moment(dateValue, calendarSetting.momentFormat)._d;
            console.log('momented:  ', dateValue);
            dateValue = Date.parse(angular.copy(dateValue));
            console.log('parsed:  ', dateValue);
            dateValue = $filter('date')(angular.copy(dateValue), 'yyyy-MM-dd');
            console.log('angualraised:  ', dateValue);
            return dateValue;
        },
...
.service('CommonUtils', function($q, $translate, $filter, SessionStorageService, DateUtils, OptionSetService, CurrentSelection, FileService, DialogService, OrgUnitService){
    ...
    return {
        ...
        userHasReadAccess: function( storage, objectId ){
            var objs = SessionStorageService.get(storage);
            if (objs && objs.length) {
                for (var i = 0; i < objs.length; i++) {
                    if (objs[i].id === objectId && objs[i].access && objs[i].access.data && objs[i].access.data.read) {
                        return true;
                    }
                }
            }
